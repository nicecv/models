...
tf.app.flags.DEFINE_integer(
    'pred_image_size', None, 'Pred image size')
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    pred_image_size = FLAGS.pred_image_size or network_fn.default_image_size

    image = image_preprocessing_fn(image, pred_image_size, pred_image_size)
    ...
    tf.logging.info('Predciting %s' % checkpoint_path)
    
    coord = tf.train.Coordinator()
    step = 0
    if num_batches*FLAGS.batch_size > 2000000:
      tf.logging.info('Too many samples to predict %d' % num_batches*FLAGS.batch_size)
    predictions = np.zeros((num_batches*FLAGS.batch_size, dataset.num_classes))
    ...
    with tf.Session() as sess:
      ...
      try:    
        while step < num_batches and not coord.should_stop():
          [logits_value, labels_value] = sess.run([logits, labels])
          print (step, num_batches)
          bsize = logits_value.shape[0]
          predictions[curr_pos:curr_pos+bsize,:] = logits_value
          targets[curr_pos:curr_pos+bsize,:] = labels_value
          curr_pos += bsize
          step += 1
